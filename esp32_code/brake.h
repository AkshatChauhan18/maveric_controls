...
// Returns the estimated brake actuator position in milliseconds (0 = fully retracted, BRAKE_PULSE_MS = fully extended)
unsigned long getBrakePositionMs();

// Resets/re-zeroes the estimated brake position
void resetBrakePosition(unsigned long posMs = 0);
